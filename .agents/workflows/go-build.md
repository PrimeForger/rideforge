...
When resolving build blockers tracked in `docs/STATUS.md`:
- Only mark a build/compilation blocker resolved after `go build ./...` and `go vet ./...` exit with code 0.
- If persistent toolchain/compiler issues remain (e.g. CGO linker flag issues), ensure they are clearly documented under "Active Blockers & Known Issues".
